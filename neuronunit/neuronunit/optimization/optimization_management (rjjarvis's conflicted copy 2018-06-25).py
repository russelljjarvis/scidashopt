...
def update_exhaust_pop(pop, tests, td, backend = None):
    ...
    for i,d in zip(dtcpop,pop):
        assert list(i.attrs.values()) in list(d.dtc.values())
...
def update_deap_pop(pop, tests, td, backend = None):
    ...
    for i,d in zip(dtcpop,pop):
        print(list(i.attrs.values()),list(d.dtc.attrs.values()))
        print(list(i.attrs.values()) in list(d.dtc.attrs.values()))
        #print(list(i.attrs.values())[0] in list(d.dtc.attrs.values())[0])
